...
/// floating transcript bar (`FloatingBarView`) with fake state, composed on the dark
/// background of the app icon. Used for blog hero images so they look exactly like the app.
///
...
enum HeroRender {
    ...
    static func run(arguments: [String]) async -> Int32 {
        ...
        let pill = PillView(state: state).environment(\.colorScheme, .dark)
        ...
        // Compose: dark gradient (app icon), a menu bar strip with the pill at the right, the bar under it.
        ...
        let menuH = 24 * scale
        ctx.setFillColor(rgb(255, 255, 255, 0.07)); ctx.fill(CGRect(x: 0, y: H - menuH, width: W, height: menuH))
        ctx.setFillColor(rgb(255, 255, 255, 0.10)); ctx.fill(CGRect(x: 0, y: H - menuH, width: W, height: 1.5 * scale / 2))
        ...
        for w in [6, 7, 8, 11] as [CGFloat] {
            ...
            ctx.addPath(CGPath(roundedRect: r, cornerWidth: scale, cornerHeight: scale, transform: nil)); ctx.setFillColor(rgb(255, 255, 255, 0.28)); ctx.fillPath()
            gx -= (w + 7) * scale
        }
        ...
        ctx.saveGState()
        ctx.setShadow(offset: CGSize(width: 0, height: -6 * scale / 2), blur: 24 * scale / 2, color: rgb(0, 0, 0, 0.55))
        ctx.draw(barImg, in: barRect)
        ctx.restoreGState()

        if !caption.isEmpty {
            let capAttrs: [NSAttributedString.Key: Any] = [.font: NSFont.systemFont(ofSize: 17 * scale, weight: .medium), .foregroundColor: NSColor.white.withAlphaComponent(0.6)]
            ...
            if !sub.isEmpty {
                let subAttrs: [NSAttributedString.Key: Any] = [.font: NSFont.systemFont(ofSize: 12 * scale), .foregroundColor: NSColor.white.withAlphaComponent(0.32)]
