...
Transcriber records two audio streams on your Mac, the microphone and the system output, transcribes each one locally with a downloaded model, and writes the result to a folder while the call is still going. Different speakers are already separated: what the microphone hears is you, what the speakers play is everyone else. This post walks through each step, including the parts that are still rough.

## Where does the audio come from?

From two places that macOS treats very differently. Your own voice arrives through the microphone, which any app can record after you allow it once. The other participants arrive as sound that Zoom, Teams, WhatsApp or a browser sends to your speakers, and for years there was no clean way for another app to listen to that. People installed virtual audio drivers, which worked until the next macOS update.
...
The first split is naturally free and already in place: Everything from the microphone channel gets your name, which you set in Settings, and everything from the system channel is the other side. For a one-on-one call that is already the whole answer.

When several people are on the far end, the diarization step runs over the system channel after the call. It listens for distinct voices and splits the channel into Speaker 1, Speaker 2 and so on. What it can't do is know their names, because there is no meeting platform telling it who is who. You get labels, and you rename them if you need to (there is an experimental feature which could figure out the names live during the call, but yeah, experimental).
...
The transcript is a machine transcript. Names, product codes and heavy accents want a read-through. Speaker labels on the far end are labels, not names. And it only runs on a Mac, because the whole approach depends on the macOS audio tap.
